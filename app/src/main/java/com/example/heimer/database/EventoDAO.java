...
public class EventoDAO {
    ...
    public List<Evento> busca(String string){
        String sql = "SELECT evento._id, evento.nome, evento.data, evento.local, local.nome, local.bairro, local.cidade, local.capacidade FROM " +
        EventoEntity.TABLE_NAME +  " INNER JOIN " + LocalEntity.TABLE_NAME + " ON " + EventoEntity.TABLE_NAME + "." + EventoEntity.COLUMN_NAME_LOCAL + " = " +
                LocalEntity.TABLE_NAME + "." + LocalEntity._ID + " WHERE " + EventoEntity.COLUMN_NAME_NOME + " LIKE %" + string + "%";
